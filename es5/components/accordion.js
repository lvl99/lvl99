    ...
    value: function init() {
        ...
        if (!this.getAttr('$items').length) {
          ...
          $items.each(function (i, item) {
            ...
            if (!$item.attr('data-component-id')) {
              accordionToggleableItem = _this2._app.createComponentInstance('LVL99:ToggleableComponent', merge({
      ...
      if (this.getAttr('a11y')) {
        this.getElem().on('focus', this.getAttr('_accordionItemSelector') + ' ' + this.getAttr('_accordionItemToggleSelector'), function (jQueryEvent) {
          var $item = $(jQueryEvent.target).closest(_this2.getAttr('_accordionItemSelector'));
      ...
      this.getElem().on(events.inputstart, this.getAttr('_accordionItemSelector') + ' ' + this.getAttr('_accordionItemToggleSelector'), function (jQueryEvent) {
        var $item = $(jQueryEvent.target).closest(_this2.getAttr('_accordionItemSelector'));
        jQueryEvent.preventDefault();
        $item.trigger('LVL99:ToggleableComponent:open');
        _this2.setActiveItem($item);
